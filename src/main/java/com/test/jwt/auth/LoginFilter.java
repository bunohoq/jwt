...
public class LoginFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
		...
		//위의 정보를 AuthenticationManager에 전달 + DTO
		UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(username, password);
		
		//AuthenticationManager에게 인증해달라고 요청
		//- AuthenticationManager > CustomUserDetailsService 호출(loadUserByUsername 메서드)
		return authManager.authenticate(authToken);
	}
	
	
	//attemptAuthentication()의 결과 중 인증(ID/PW 올바름) 성공했을 때 이 메서드가 호출된다.
	@Override
